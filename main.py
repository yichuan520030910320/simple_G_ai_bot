...
Usage:
    python main_single_player.py --mode data --samples 50    # Collect data
    python main_single_player.py --mode benchmark --models gpt-4o claude-3.5-sonnet  # Run benchmark
    python main_single_player.py --mode interactive --model gpt-4o  # Interactive testing
"""
        ...
        for turn in range(turns):
            ...
            try:
                ...
                if location is not None:
                    ...
                    bot.select_map_location(x=bot.map_x + bot.map_w//2, 
                                          y=bot.map_y + bot.map_h//2, plot=plot)
...
def data_collection_mode(samples: int = 50, headless: bool = False):
    """Data collection mode"""
    print(f"📊 Starting data collection mode - {samples} samples")
    
    with DataCollector(headless=headless) as collector:
        data = collector.collect_samples(samples)
        print(f"✅ Collected {len(data)} samples successfully")


def benchmark_mode(models: list = None, samples: int = 10, live: bool = False, headless: bool = False):
    """Benchmark mode"""
    if models is None:
...
Examples:
  ...
  python main_single_player.py --mode benchmark --live --models gpt-4o
        """
    )
    
    parser.add_argument('--mode', choices=['interactive', 'data', 'benchmark'], 
                       default='interactive', help='Operation mode')
    
    # Interactive mode options
    parser.add_argument('--model', choices=list(MODELS_CONFIG.keys()), 
                       default='gpt-4o', help='Model for interactive mode')
    parser.add_argument('--turns', type=int, default=5, 
                       help='Number of turns in interactive mode')
    parser.add_argument('--plot', action='store_true', 
                       help='Generate plots of predictions')
    
    # Data collection options
    parser.add_argument('--samples', type=int, default=50, 
                       help='Number of samples to collect/test')
    
    # Benchmark options  
    parser.add_argument('--models', nargs='+', choices=list(MODELS_CONFIG.keys()),
                       help='Models to benchmark')
    parser.add_argument('--live', action='store_true',
                       help='Use live MapCrunch website for benchmark')
